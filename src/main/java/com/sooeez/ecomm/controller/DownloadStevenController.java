...
public class DownloadStevenController
{
		...
		@RequestParam( value = "ids", required = false ) Long[] ids) throws IOException
	{
		...
		{
			excelName = "发货单( " +
				id + " ) - " + ( new SimpleDateFormat( "yyyy-MM-dd_HH:mm:ss" ) ).format( new Date() ) + ".xls";
			isExportable = true;
		}
		...
		String[] titles =
		{
			"收货人", "电话号", "手机号", "地址", "物品内容", "邮编", "快递单号", "发货单号", "状态", "备注"
		};
		...
		@RequestParam( value = "ids", required = false ) Long[] ids) throws IOException
	{
		...
		{
			excelName = "采购单( " +
				id + " ) - " + ( new SimpleDateFormat( "yyyy-MM-dd_HH:mm:ss" ) ).format( new Date() ) + ".xlsx";
			isExportable = true;
		}

		// else if( ids != null && ids.length > 0 )
		// {
		// excelName = "采购单( 批量 ) - " + (new
		...
		// {
		// List<Shipment> shipments = this.shipmentService.getShipmentsByIds(
		// ids );
		//
		// if( shipments != null && shipments.size() > 0 )
		// {
